    ...
    public function sendMessage()
    {
        ...
        if($type == "friend"){
            ...
            if($examine){
                ...
                //获取组内所有成员，然后单独转发
                $atgroup = db("group")->where("id",$to)->find();
                if($atgroup["grouplist"]){
                    foreach (json_decode($atgroup["grouplist"]) as $key => $value) {
                       // 如果不在线就先存起来
                        if(!Gateway::isUidOnline($value))
                        {// 假设有个your_store_fun函数用来保存未读消息(这个函数要自己实现)
                            $OnlineArr=[
                                 'username'=> $username //消息来源用户名
                                ,'avatar'=> $avatar //消息来源用户头像
